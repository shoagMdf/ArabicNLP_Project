...
#NLP loader
# ==============================================================================
# Download Arabic model (only the first time)
model_dir = r"C:\pycharmCodes\NLP\Githup_NLP\Githup\scripts"
os.makedirs(model_dir, exist_ok=True)
#stanza.download('ar', model_dir=model_dir)
# Load the Arabic NLP pipeline globally
nlp = stanza.Pipeline(
    lang='ar',
...
def DataPreparation():
    ...
    BASE_DIR = r"C:\pycharmCodes\NLP\Githup_NLP\Githup"
    os.chdir(BASE_DIR)
    data_dir_raw = setup_directory(os.path.join(BASE_DIR,"data","raw"))
    data_dir_processed = setup_directory(os.path.join(BASE_DIR,"data","Processed"))
    by_polishing, from_title, from_title_and_content = load_datasets(data_dir_raw)
    ...
    save_to_excel(dfs, sheet_names, os.path.join(data_dir_processed, 'reshaped_datasets.xlsx'))
   
    concatenated_df = concatenate_and_save(dfs, os.path.join(data_dir_processed, 'concatenated_data.xlsx'))
...
def perform_lexical_analysis(df: pd.DataFrame):
    ...
    file_path = os.path.join(results_dir, "lexical_analysis_comparison.csv")
    lexical_df.to_csv(file_path, index=False, encoding="utf-8-sig")
    print(f"\nLexical analysis comparison saved to '{file_path}'")
...
MAX_LENGTH = 256
USE_GPU = True
OUTPUT_DIR = r"C:\pycharmCodes\NLP\Githup_NLP\Githup\data\Processed\Data_Split"
